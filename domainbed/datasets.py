...
class Spirals(MultipleDomainDataset):
    ...
    def __init__(self, root, test_env, hparams):
        super().__init__()
        self.datasets = []
        environments = [i for i in range(16)]       # HERE

        test_dataset = self.make_tensor_dataset(env='test')
        self.datasets.append(test_dataset)
        for env in environments:
            env_dataset = self.make_tensor_dataset(env=env, seed=np.random.get_state()[1][0]+env)       #HERE SEED
            self.datasets.append(env_dataset)
        
        self.input_shape = (18,)
        self.num_classes = 2

    def make_tensor_dataset(self, env, n_examples=1024, n_envs=16, n_revolutions=3, n_dims=16,      # HERE N_DIMS=16
